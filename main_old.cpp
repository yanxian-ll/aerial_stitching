...
// ======================== idle watcher ========================
void idleWatcherThread(int idle_timeout) {
                   ...
                   int /*project_id*/)
{
    ...
    while (!exitFlag.load()) {
        ...
        pool.enqueue([currentImage, currentMetadata, image_filename,
                      &outputFolder, &committedCount, &lastSaveTime,
                      seam_mode, blend_mode, save_interval, match_threshold, maxIdle, max_seam_width]() mutable
        {
            ...
            // 5) 提交阶段：接缝 + DOM 更新 + 边界更新
            {
                ...
                auto now = Clock::now();
                if (!needSave && now - lastSaveTime > maxIdle) needSave = true;
...
int main(int argc, char *argv[]) {
    ...
    int c;
    while ((c = getopt_long(argc, argv, "i:o:p:s:b:r:m:n:t:x:h", longopts, NULL)) != -1) {
        switch (c) {
    ...
    std::thread t_idle;
    if (idle_timeout > 0) {
        t_idle = std::thread(idleWatcherThread, idle_timeout);
    }
